# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from __future__ import annotations

import os
from typing import Any, cast

import pytest

from tests.utils import assert_matches_type
from prediction_guard import PredictionGuard, AsyncPredictionGuard
from prediction_guard.types import InjectionCreateResponse

base_url = os.environ.get("TEST_API_BASE_URL", "http://127.0.0.1:4010")


class TestInjection:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    def test_method_create(self, client: PredictionGuard) -> None:
        injection = client.injection.create(
            detect=True,
            prompt="A short poem may be a stylistic choice or it may be that you have said what you intended to say in a more concise way.",
        )
        assert_matches_type(InjectionCreateResponse, injection, path=["response"])

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    def test_raw_response_create(self, client: PredictionGuard) -> None:
        response = client.injection.with_raw_response.create(
            detect=True,
            prompt="A short poem may be a stylistic choice or it may be that you have said what you intended to say in a more concise way.",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        injection = response.parse()
        assert_matches_type(InjectionCreateResponse, injection, path=["response"])

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    def test_streaming_response_create(self, client: PredictionGuard) -> None:
        with client.injection.with_streaming_response.create(
            detect=True,
            prompt="A short poem may be a stylistic choice or it may be that you have said what you intended to say in a more concise way.",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            injection = response.parse()
            assert_matches_type(InjectionCreateResponse, injection, path=["response"])

        assert cast(Any, response.is_closed) is True


class TestAsyncInjection:
    parametrize = pytest.mark.parametrize(
        "async_client", [False, True, {"http_client": "aiohttp"}], indirect=True, ids=["loose", "strict", "aiohttp"]
    )

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    async def test_method_create(self, async_client: AsyncPredictionGuard) -> None:
        injection = await async_client.injection.create(
            detect=True,
            prompt="A short poem may be a stylistic choice or it may be that you have said what you intended to say in a more concise way.",
        )
        assert_matches_type(InjectionCreateResponse, injection, path=["response"])

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    async def test_raw_response_create(self, async_client: AsyncPredictionGuard) -> None:
        response = await async_client.injection.with_raw_response.create(
            detect=True,
            prompt="A short poem may be a stylistic choice or it may be that you have said what you intended to say in a more concise way.",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        injection = await response.parse()
        assert_matches_type(InjectionCreateResponse, injection, path=["response"])

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    async def test_streaming_response_create(self, async_client: AsyncPredictionGuard) -> None:
        async with async_client.injection.with_streaming_response.create(
            detect=True,
            prompt="A short poem may be a stylistic choice or it may be that you have said what you intended to say in a more concise way.",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            injection = await response.parse()
            assert_matches_type(InjectionCreateResponse, injection, path=["response"])

        assert cast(Any, response.is_closed) is True
