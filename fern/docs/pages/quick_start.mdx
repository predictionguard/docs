...
const client = new pg.Client('<Your PG API URL>', process.env.PREDICTIONGUARD_API_KEY);
...
```bash
curl -i -X POST <Your PG API URL>/chat/completions     -H "Authorization: Bearer ${PREDICTIONGUARD_API_KEY}"     -H "Content-Type: application/json"     -d '{
    "model": "{{TEXT_MODEL}}",
    "messages": [
        {
            "role": "system",
